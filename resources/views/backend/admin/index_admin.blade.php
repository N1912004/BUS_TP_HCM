...
<div class="container-fluid">
  <div class="row g-0">
    ...
    <aside class="col-lg-2 sidebar p-0">
      <div class="brand">
        <img src="{{ asset('backend/logo/logo.png') }}" alt="Logo" style="width:36px;height:36px;" class="me-2">
      ...
      <nav class="nav flex-column p-3">
        <div class="dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="userManagementDropdown">
            <li><a class="dropdown-item" href="{{ route('admin.drivers.index') ?? '#' }}">Tài xế</a></li>
            <li><a class="dropdown-item" href="{{ route('admin.assistants.index') ?? '#' }}">Phụ xe</a></li>
          </ul>
        </div>
        <a class="nav-link d-flex align-items-center mb-2" href="{{ route('admin.tickets.index') ?? '#' }}">
          <i class="fa fa-ticket-alt me-2"></i> Quản lý vé
        </a>
        <a class="nav-link d-flex align-items-center mb-2 active" href="{{ route('admin.routes.index') ?? '#' }}">
          <i class="fa fa-route me-2"></i> Quản lý tuyến xe
        </a>
         <a class="nav-link d-flex align-items-center mb-2 active" href="{{ route('admin.buses.index') ?? '#' }}">
      ...
      <header class="header-top d-flex justify-content-between align-items-center px-4" style="height:64px;">
        ...
        <div class="d-flex align-items-center">
          <div class="me-3 text-end pe-3 border-end">
            ...
            <div class="fw-bold" style="font-size:14px;">{{ Auth::user()->name ?? 'Quản trị viên' }}</div>
          </div>

        <!-- HOME PAGE DISPLAY (Giống hình bạn gửi) -->
{{-- <div class="dashboard-banner">
  <div class="container">
    <img src="{{ asset('backend/img/bus_illustration.png') }}"
         alt="BusGo HCM Illustration">
  </div>
</div> --}}
                ...
                <div class="widget style1 navy-bg">
                    <div class="row">
                        ...
                        <div class="col-xs-8 text-right">
                            ...
                            <h2 class="font-bold" id="totalRoutes">{{ $totalRoutes }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="widget style1 lazur-bg">
                    <div class="row">
                        ...
                        <div class="col-xs-8 text-right">
                            ...
                            <h2 class="font-bold" id="totalBuses">{{ $totalBuses }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="widget style1 yellow-bg">
                    <div class="row">
                        ...
                        <div class="col-xs-8 text-right">
                            ...
                            <h2 class="font-bold" id="totalUsers">{{ $totalUsers }}</h2>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="widget style1 red-bg">
                    <div class="row">
                        ...
                        <div class="col-xs-8 text-right">
                            ...
                            <h2 class="font-bold" id="totalDrivers">{{ $totalDrivers }}</h2>
                        </div>
                    </div>
                </div>
            </div>
      ...
      {{-- <div class="dashboard-banner">
          <img src="{{ asset('backend/img/bus_illustration.png') }}" alt="BusGo HCM Illustration">
      </div> --}}
        @yield('content')
      </section>
...
<!-- Hidden logout form -->
<form id="logout-form" action="{{ route('auth.logout') ?? url('/logout') }}" method="POST" style="display:none;">
  @csrf
</form>
...
<script>
    $(document).ready(function() {
        function updateStats() {
            $.ajax({
                url: '{{ route('api.admin.stats') }}', // This route needs to be defined
                method: 'GET',
